...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const unsubFollowers = onSnapshot(followersQuery, (snap) => {
      ...
      console.error("Followers snapshot error:", error);
                    ...
                    <div className="w-full h-full relative group/photo">
                       {isUploading && (
                         <div className="absolute inset-0 bg-black/20 backdrop-blur-[2px] flex items-center justify-center z-10 transition-all">
                            <div className="w-6 h-6 border-2 border-white border-t-transparent rounded-full animate-spin shadow-sm"></div>
                         </div>
                       )}
                       ...
                       <button 
                         ...
                         className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center text-white opacity-0 group-hover/photo:opacity-100 transition-opacity disabled:pointer-events-none"
                       >
                         <Camera className="w-6 h-6 mb-1" />
                         <span className="text-[10px] font-bold uppercase">{isUploading ? 'Uploading...' : 'Change'}</span>
                       </button>
                    </div>
                  ) : profile.photoURL ? (
                  ...
                  {isEditing ? (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex gap-2">
                        <button 
                          onClick={() => initiateCall(targetUid!, profile.displayName!, 'voice')}
                          className="w-10 h-10 bg-white border border-gray-200 text-[#0A2F6F] rounded-lg shadow-sm hover:bg-gray-50 flex items-center justify-center transition-all"
                          title="Voice Call"
                        >
                          <Phone className="w-4 h-4" />
                        </button>
                        <button 
                          onClick={() => initiateCall(targetUid!, profile.displayName!, 'video')}
                          className="w-10 h-10 bg-white border border-gray-200 text-[#10A37F] rounded-lg shadow-sm hover:bg-gray-50 flex items-center justify-center transition-all"
                          title="Video Call"
                        >
