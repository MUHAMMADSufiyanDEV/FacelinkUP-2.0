...
export default function Gigs() {
  ...
  const gigFileInputRef = React.useRef<HTMLInputElement>(null);
  ...
  const handleCreateGig = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, 'gigs'), {
        ...
        images: newGig.imageUrl ? [newGig.imageUrl] : [],
        rating: 5.0,
        reviewsCount: 0,
        createdAt: serverTimestamp()
      });
      setIsModalOpen(false);
      setNewGig({ title: '', description: '', price: '', category: 'Design', deliveryDays: '3', imageUrl: '' });
    } catch (error) {
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleCreateGig} className="space-y-6">
                ...
                <div>
                   <label className="text-sm font-bold text-[#0A2F6F] mb-2 block">Gig Image</label>
                   <div className="space-y-4">
                     <div className="flex gap-4">
                       <input 
                         ...
                         onChange={async (e) => {
                           ...
                           try {
                             ...
                             setNewGig({ ...newGig, imageUrl: url });
                           } catch (err) {
                             console.error("Gig upload failed:", err);
                       ...
                       <button
                         ...
                         className="flex-1 py-3 bg-white border border-[#E9ECEF] text-[#0A2F6F] font-bold rounded-xl hover:bg-gray-50 transition-all flex items-center justify-center gap-2 shadow-sm disabled:opacity-50"
                       >
                     ...
                     {newGig.imageUrl && (
                       <div className="relative aspect-video rounded-2xl overflow-hidden border border-gray-100">
                         <img src={newGig.imageUrl} alt="Gig preview" className="w-full h-full object-cover" />
                         <button 
                           type="button"
                           onClick={() => setNewGig({ ...newGig, imageUrl: '' })}
