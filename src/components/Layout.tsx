...
const aiNavItems = [
  { icon: Sparkles, label: 'Resume Analyzer', path: '/ai/resume' },
  { icon: Wand2, label: 'Resume Maker', path: '/ai/resume-maker' },
  { icon: Target, label: 'Job Matcher', path: '/ai/job-matcher' },
  { icon: Send, label: 'Proposal Gen', path: '/ai/proposal' },
  { icon: MessageSquare, label: 'Interview Prep', path: '/ai/interview' },
  { icon: Video, label: 'Live Interview', path: '/ai/interview/live' },
  { icon: Mail, label: 'Email Writer', path: '/ai/email' },
  { icon: ListChecks, label: 'Daily Planner', path: '/ai/tasks' },
];
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const isHardcodedAdmin = normalizedEmail === 'm.sufiyan1581@gmail.com' || normalizedEmail === 'sufiyangaming911@gmail.com';
  const effectiveIsAdmin = isAdmin || isHardcodedAdmin;

  const currentNavItems = [
    ...(effectiveIsAdmin ? [{ icon: ShieldAlert, label: 'ADMIN PANEL', path: '/admin' }] : []),
    ...navItems,
        ...
        <div className="p-6 flex flex-col h-full">
          ...
          <nav className="flex-1 px-2 space-y-1 overflow-y-auto custom-scrollbar py-2">
            ...
            <div className="pt-4 pb-2 px-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">AI Power Tools</p>
            </div>
            ...
            {aiNavItems.map((item) => (
              <Link
                ...
                className={cn(
                  "flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-sm font-medium",
                  location.pathname === item.path 
                    ? "bg-blue-50 text-[#0A2F6F]" 
                    : "text-[#6C757D] hover:bg-blue-50/50 hover:text-[#0A2F6F]"
                )}
              >
                <item.icon className={cn("w-5 h-5", location.pathname === item.path ? "text-[#0A2F6F]" : "text-blue-400")} />
                {item.label}
              </Link>
            ))}
          </nav>
